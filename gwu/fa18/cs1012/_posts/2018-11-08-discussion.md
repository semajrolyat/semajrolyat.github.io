...
Variables of type ```int``` and ```float``` are mutable.  We introduced the ```string``` type as immutable.  We will revisit this concept with regard to various sequence types in this discussion.
...
We can access elements in the list using the same square brackets, _i.e._ ```[``` and ```]```, that we used to create the list.  It is important to remember that we count from ```0``` in programming, so the first element is at position ```0```, the second element is at position ```1```, and the last element is at position ```n-1```.  For example, the following code accesses various elements in the list:
